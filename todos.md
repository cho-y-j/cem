...
**발견된 문제:**
1. **Worker 삭제 시 Foreign Key Constraint 에러**
   - `entry_request_items.paired_worker_id`가 `workers.id`를 참조하여 삭제 불가
   - 에러: "Unable to delete row as it is currently referenced by a foreign key constraint"

2. **Owner 화면에서 생성한 Worker가 보이지 않음**
   - `getWorkersWithFilters`에서 존재하지 않는 `owner_company_id` 컬럼 필터링 시도
   - `workers` 테이블에는 `owner_id`만 있고 `owner_company_id`는 없음

3. **사용자 관리 시스템의 Supabase Auth와 DB 동기화 문제**
   - UUID가 아닌 사용자 삭제 시 Auth 삭제를 건너뛰어 불완전한 삭제 발생

**해결 방법:**
1. ✅ **Worker 삭제 로직 개선** (`server/db.ts:1029-1061`):
   - 삭제 전 `entry_request_items`에서 `paired_worker_id`를 `NULL`로 설정
   - 에러 처리 개선 (throw Error로 변경)
   - 로그 추가

2. ✅ **Owner Worker 필터링 수정** (`server/db.ts:925-941`):
   - 존재하지 않는 `owner_company_id` 필터 제거
   - `ownerId` 필터만 사용하도록 수정
   - TODO 주석 추가 (향후 개선 방안)

3. ✅ **사용자 삭제 로직 개선** (`server/users-router.ts:358-391`):
   - UUID가 아닌 경우에도 이메일로 Auth 사용자 찾아 삭제 시도
   - Auth 삭제 실패해도 DB 삭제는 계속 진행 (레거시 사용자 대응)
   - 에러 처리 개선 (warn 로그로 변경)

**수정된 파일:**
- `server/db.ts` - `deleteWorker`, `getWorkersWithFilters` 함수 개선
- `server/users-router.ts` - 사용자 삭제 로직 개선
- `PROBLEM_ANALYSIS_USER_MANAGEMENT.md` - 문제 분석 문서 작성

**결과:**
- ✅ Worker 삭제 시 Foreign Key 에러 해결
...
### ✅ Inspector 모바일에서 NFC 태그 등록 기능 추가

**목표:**
- 현장 안전관리자가 바로 장비에 붙인 태그를 시스템과 연결할 수 있도록 UX 개선

**구현 내용:**
- ✅ NFC 스캔 실패 시 안내 개선: 미등록 태그 감지 시 토스트와 함께 태그 값을 저장해두고 카드의 `태그 등록` 버튼으로 이어지도록 구성
- ✅ 검색 결과 카드 UI 개편
  - 배정 운전자 / BP / Owner / 기존 태그 정보를 한눈에 확인
  - `태그 등록`, `점검 시작` 액션을 카드 하단 두 개 버튼으로 배치
  - 카드 우측 화살표 클릭 시 여전히 점검 화면으로 이동
- ✅ NFC 태그 등록 모달
  - 장비 요약 정보, 태그 입력 필드, `태그 스캔` 버튼, `태그 저장` 버튼 제공
  - 안드로이드 Chrome 한정으로 Web NFC 스캔 지원
- ✅ 중복 태그 체크 및 API
  - `assignNfcTagToEquipment` DB 유틸 추가 → 중복 태그일 경우 예외 처리
  - `safetyInspection.assignNfcTag` tRPC mutation으로 Inspector도 태그 등록 가능
- ✅ 로컬 상태 업데이트: 저장 후 검색 결과 카드의 태그 값 즉시 반영

**주의 사항 / 후속 작업:**
- Admin/Owner UI에서도 태그를 등록/수정할 수 있도록 별도 화면 추가 예정 (`task-nfc-admin-register`)
- 태그 해제(삭제)는 아직 제공하지 않음 → 필요 시 API/UX 확장 예정

**상태:** ✅ **완료 및 Inspector 실사용 준비 완료**
...
**개요:**
- 관리자/Owner가 백오피스에서 장비 정보를 편집할 때 NFC 태그를 함께 관리할 수 있도록 확장

**주요 변경 사항:**
- ✅ 장비 등록/수정 폼에 `NFC 태그` 입력 필드 추가 (`client/src/pages/Equipment.tsx`)
  - 유효성: 중복 체크, 공백 입력 시 미등록 처리
  - UI: 안내 문구/placeholder로 태그 입력 가이드 제공
- ✅ 장비 목록/상세 화면에 태그 표시
  - 목록 테이블에 `NFC 태그` 컬럼 추가 (배지 스타일)
  - 상세 다이얼로그 기본 정보 카드에 태그 배지 표시
- ✅ 서버 로직 확장 (`server/routers.ts`)
  - 장비 생성/수정/서류 동반 등록 API에서 `nfcTagId` 지원
  - 수정 시 `assignNfcTagToEquipment`를 재사용하여 중복 방지 및 공통 로직 활용

**UX 메모:**
- 태그 미등록 상태 표시는 `미등록` 텍스트로 통일
- Inspector와 동일하게 중복 태그 입력 시 사용자에게 명확한 에러 토스트 제공

**상태:** ✅ **완료 및 관리자/Owner UI 반영**

---

### ✅ Inspector 검색 화면 UI & NFC 경험 개선

**개선 포인트:**
- ✅ 페이지 진입 시 Web NFC를 자동으로 시도하고, 스캔 실패 시 안내 토스트 및 버튼으로 재시도 가능
- ✅ 상단 안내 영역을 간결화하고, 검색 입력 UI 높이/타이포를 정리하여 모바일에서 겹침 해소
- ✅ 검색 결과 카드 레이아웃 재구성
  - 차량번호·장비 종류·규격을 한 줄에 정리
  - 배정 인력/회사 뱃지가 한 줄에 표시되도록 flex-wrap 적용
  - `점검 시작` 버튼을 기본(primary) 스타일, `태그 등록`은 outline로 구분
  - 등록되지 않은 태그 스캔 시 카드 상단에 경고 배너 표시
- ✅ 점검 프로세스/주의사항 카드 디자인 정돈: 번호형 단계와 강조 카드로 가독성 향상
