  ...
  return new Promise((resolve, reject) => {
    ...
    reader.onload = (e) => {
      img.onload = () => {
        ...
        // OCR 결과에서 주민번호 위치 정보가 있으면 사용
        if (ocrInfo.residentNumberBounds) {
          maskX = img.width * ocrInfo.residentNumberBounds.x;
          maskY = img.height * ocrInfo.residentNumberBounds.y;
          maskWidth = img.width * ocrInfo.residentNumberBounds.width;
          maskHeight = img.height * ocrInfo.residentNumberBounds.height;
          console.log('[Masking] OCR 위치 정보 사용:', { 
          ...
          // 하이픈 뒤 7자리 + 앞뒤 여유 공간 (하이픈은 마스킹하지 않음)
          // 면허증마다 레이아웃이 다를 수 있으므로 넓은 영역으로 마스킹
          maskX = img.width * 0.50; // 왼쪽에서 50% 지점 (하이픈 뒤 시작, 여유 공간 포함)
          maskY = img.height * 0.34; // 위에서 34% 지점 (주민번호 위치, 다양한 면허증 대응)
          maskWidth = img.width * 0.22; // 너비의 22% (7자리 + 앞뒤 넓은 여유)
          maskHeight = img.height * 0.07; // 높이의 7% (더 높은 영역으로 다양한 면허증 대응)
          console.log('[Masking] 기본 위치 사용:', { 
            maskX: maskX.toFixed(0), 
            maskY: maskY.toFixed(0), 
            maskWidth: maskWidth.toFixed(0), 
            maskHeight: maskHeight.toFixed(0),
        ...
        console.log('[Masking] 주민번호 뒷자리 마스킹 완료 (검은색 사각형, 앞뒤 포함)');
